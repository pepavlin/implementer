...
describe("buildDashboardData", () => {
    ...
    it("places running tasks at top, then queued, then completed (status priority order)", () => {
        ...
        // running task must come first (highest priority)
        expect(taskList[0].taskId).toBe("running1");
        expect(taskList[0].status).toBe("running");
        // queued comes second
        expect(taskList[1].taskId).toBe("queued1");
        expect(taskList[1].status).toBe("queued");
        // completed is last
        expect(taskList[2].taskId).toBe("completed1");
        expect(taskList[2].status).toBe("completed");
    });

    it("places starting tasks above queued and completed", () => {
        const tasks: Task[] = [
            makeTask({
                taskId: "completed1" as TaskId,
                status: "completed",
        ...
        // starting comes before queued (priority 1 vs 2)
        expect(taskList[0].taskId).toBe("starting1");
        expect(taskList[0].status).toBe("starting");
        expect(taskList[1].taskId).toBe("queued1");
        expect(taskList[1].status).toBe("queued");
        // completed is last
        expect(taskList[2].taskId).toBe("completed1");
    ...
    it("places running tasks above starting, queued and completed (full priority order)", () => {
        const tasks: Task[] = [
            makeTask({
                taskId: "completed1" as TaskId,
                status: "completed",
        ...
        // Full priority order: running > starting > queued > completed
        expect(taskList[0].taskId).toBe("running1");
        expect(taskList[1].taskId).toBe("starting1");
        expect(taskList[2].taskId).toBe("queued1");
        expect(taskList[3].taskId).toBe("completed1");
    });
