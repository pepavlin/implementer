...
export type WorkspaceId = number & { readonly _: unique symbol };
...
export class WorkspacePool {
    ...
    private instances: Map<WorkspaceId, WorkspaceInstance> = new Map();
    private nextId = 0;
...
`;
    ...
    initFromDisk(): void {
        ...
        for (const entry of entries) {
            ...
            const id = parseInt(entry.name, 10) as WorkspaceId;
    ...
    async acquireExisting(
        ...
        console.log(
            `[pool] Acquired existing workspace instance ${id} for resumption`
        );
