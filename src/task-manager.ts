...
export function buildSystemInstructions(repos: { name: string }[], protectedPaths?: string[]): string {
...
- NEVER close, merge, delete, or otherwise change the state of pull requests. You are STRICTLY FORBIDDEN from running \`gh pr close\`, \`gh pr merge\`, \`gh pr delete\`, or any other command that would close, merge, or remove a pull request. A technical enforcement layer will block these commands even if you try to run them. Your only allowed PR action is creating new pull requests with \`gh pr create\` — and even that is handled automatically by the system after your work is complete, so you should not run it yourself either.
